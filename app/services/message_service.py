...
## START - added from previous version -
    async def handle_incoming_whatsapp_message_v1(
        self,
        phone_number: str,
        ...
        message_history = self.history_service.get_message_history_v1(conversation_id=conversation.id, limit=10)
        ...
        result = await run_manager_legacy(formatted_history, phone_number)

        # Log the outgoing response
        self.history_service.log_outgoing_message_v1(
            conversation_id=conversation.id,
            content=result
        )
    ...
    async def handle_incoming_chat_message_v1(
        self,
        request: Request,
        message: str,
        ...
        # Process the message through the agent manager
        result = await run_manager_legacy(formatted_history, user.id)
    ...
    async def handle_incoming_chat_message_streaming_v1(
        self,
        request: Request,
        message: str,
        media_urls: Optional[List[str]] = None,
